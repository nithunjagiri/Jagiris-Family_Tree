...
export default function AdminUsers() {
  ...
  const load = () => {
    ...
    adminApi
      .users({
        ...
        family_access: familyAccess || undefined,
      })
      .then((r) => {
  ...
  useEffect(() => {
    load();
  }, [offset, qDebounced, role, status, familyAccess]);
  ...
  const approveAccess = async (row) => {
    setApprovingId(row.id);
    try {
      await adminApi.approveFamilyAccess(row.id);
      load();
    } catch (err) {
      alert(getApiErrorMessage(err, 'Could not approve family access.'));
    } finally {
      setApprovingId(null);
    }
  };
  ...
  return (
    <div className="w-full max-w-none space-y-6">
      <div className="flex flex-wrap items-start justify-between gap-4">
        <div>
          ...
          <p className="mt-1 max-w-3xl text-sm text-gray-600 dark:text-gray-400">
            View and manage accounts. Approve self-registered users into the shared family, edit profiles, reset
            passwords, or disable access. Disabled users cannot sign in.
          </p>
        </div>
        <Link
          to="/admin/users/new"
      ...
      <div className="flex flex-col gap-3 rounded-2xl border border-gray-200 bg-white p-4 shadow-soft dark:border-gray-800 dark:bg-gray-900 sm:flex-row sm:flex-wrap sm:items-end">
        ...
        <div>
          <label className="mb-1 block text-xs font-medium text-gray-500 dark:text-gray-400">Family access</label>
          <select
            value={familyAccess}
            onChange={(e) => {
              setFamilyAccess(e.target.value);
              setOffset(0);
            }}
            className="w-full min-w-[9rem] rounded-lg border border-gray-300 px-3 py-2 text-sm dark:border-gray-600 dark:bg-gray-800 sm:w-auto"
          >
            <option value="">All</option>
          ...
          <table className="min-w-full text-left text-sm">
            <thead className="border-b border-gray-200 bg-gray-50 dark:border-gray-800 dark:bg-gray-800/50">
              <tr>
                ...
                <th className="px-4 py-3 font-semibold text-gray-700 dark:text-gray-300">Family access</th>
                  ...
                  <tr key={row.id} className="hover:bg-gray-50 dark:hover:bg-gray-800/40">
                    ...
                    <td className="whitespace-nowrap px-4 py-2.5">
                      <span
                        className={cn(
                          'inline-flex rounded-full px-2.5 py-0.5 text-xs font-medium ring-1 ring-inset',
                          row.family_access === 'pending'
                            ? 'bg-amber-50 text-amber-900 ring-amber-600/20 dark:bg-amber-950/40 dark:text-amber-100 dark:ring-amber-500/30'
                            : 'bg-sky-50 text-sky-800 ring-sky-600/20 dark:bg-sky-950/40 dark:text-sky-200 dark:ring-sky-500/30'
                        )}
                      >
                        {row.family_access === 'pending' ? 'Pending' : 'Approved'}
                      </span>
                    </td>
                    <td className="whitespace-nowrap px-4 py-2.5 text-right">
                      <div className="flex justify-end gap-1">
                        {row.family_access === 'pending' && !row.is_admin && (
                          <button
                            type="button"
                            title="Approve family access"
                            disabled={approvingId === row.id}
                            onClick={() => approveAccess(row)}
                            className="inline-flex items-center gap-1 rounded-lg px-2 py-1.5 text-xs font-semibold text-emerald-700 hover:bg-emerald-50 disabled:opacity-50 dark:text-emerald-300 dark:hover:bg-emerald-950/40"
                          >
